...
contract BalancerLiquidity {
    ...
    function join(uint256 rethAmount, uint256 wethAmount) external {
        ...
        require(rethAmount>0,"zero balance not allowed");
        require(wethAmount>0,"zero balance not allowed");
        //1.first transferFrom and approve tokens we are providing to the protocol
        //which are WETH AND RETH
        ...
        //2.now we have to call _join fn to send these tokens to balancer vault
        //so that we must create array first as input to _join is an array of token amountsIn and assets
    ...
    function exit(uint256 bptAmount, uint256 minRethAmountOut) external {
        ...
        _exit(bptAmount,msg.sender,assets,minAmountOut);
